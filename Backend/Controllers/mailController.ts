...
export const sendMail = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    for (const row of normalizedData) {
      ...
      const htmlText = updateText(text, name, contact, linkedIn, recipientCompany, userCurrentCompany, userActualName);
      const htmlSubject = updateSubject(recipientCompany);
